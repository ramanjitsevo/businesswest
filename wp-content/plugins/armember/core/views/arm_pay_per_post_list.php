...
jQuery(document).on('keyup','#armmanagepost_search',function(e){
    if(e.keyCode == 13)
    {
        ...
        jQuery('#subscription_plans_list_form input[type="search"]').val(arm_search).trigger('keyup');
    }
})
...
function arm_load_paid_post_list_grid(is_filtered){
    ...
    var __ARM_Showing_empty = '<?php echo addslashes(esc_html__('Showing 0 to 0 of 0 Posts','ARMember')); //phpcs:ignore?>';
    ...
    var __ARM_Show = '<?php echo addslashes(esc_html__('Paid post per page','ARMember')); //phpcs:ignore?> ';
                ...
                <div class="sltstandard">
                    <div class="arm_dt_filter_block arm_datatable_searchbox">
                        <div class="arm_datatable_filter_item">
                            <label class="arm_padding_0"><input type="text" placeholder="<?php esc_attr_e( 'Search Post', 'ARMember' ); ?>" id="armmanagepost_search" value="<?php echo esc_attr($filter_search); ?>" tabindex="-1"></label>
                        </div>				
                    </div>
                </div>
            </div>
