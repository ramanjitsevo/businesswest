...
jQuery(document).on('keyup','#armmanagemembers_search',function(e){
	if(e.keyCode == 13)
	{
		...
		jQuery('input[type="search"]').val(arm_search).trigger('keyup');
	}
})
...
function arm_load_private_content_list_grid(is_filtered){
    ...
    var __ARM_Show = '<?php echo addslashes(esc_html__('Contents per page','ARMember'));//phpcs:ignore ?> ';
	...
	var table = jQuery('#armember_datatable').dataTable({
		...
		"aoColumnDefs": [
			...
			{ "sClass": 'arm_width_130', "aTargets": [0,1]},
			...
			<form method="GET" id="subscription_plans_list_form" class="data_grid_list">
				...
				<div id="armmainformnewlist">
					<table cellpadding="0" cellspacing="0" border="0" class="display arm_hide_datatable" id="armember_datatable">
						<thead>
							<tr>
								<th class="arm_width_50 arm_text_align_center"><?php esc_html_e('Enable / Disable','ARMember');?></th>
								<th class="arm_width_50 arm_text_align_center"><?php esc_html_e('User ID','ARMember');?></th>
								<th class="arm_min_width_150"><?php esc_html_e('Username','ARMember');?></th>
