    ...
    class ARM_social_feature {
        ...
        function __construct() {
            ...
            if ($is_social_login_feature == '1') {
                ...
                add_action('init', array($this, 'arm_get_global_social_settings'), 10);
        ...
        function arm_get_global_social_settings()
        {
        ...
        function arm_deactivate_plugin() {
            ...
            $ARMember->arm_check_user_cap($arm_capabilities_global['arm_manage_feature_settings'], '1',1);
        ...
        function arm_activate_plugin( $pluginslug = "", $wpnonce_check = 1 ) {

            global $ARMember, $arm_capabilities_global;
            $ARMember->arm_check_user_cap($arm_capabilities_global['arm_manage_feature_settings'], '1',$wpnonce_check);
            ...
            if (empty($plugin_slug)) {
                wp_send_json_error(array(
                    ...
                    'errorMessage' => addslashes(esc_html__('No plugin specified.', 'ARMember')),
                ));
            }
            $plugin = $plugin_slug;
            $plugin = plugin_basename(trim($plugin));
            ...
            if(!str_contains($plugin_slug_check , 'armember'))
            {
                 echo json_encode(array('error' => 'false', 'msg' => addslashes(esc_html__('You cannot install this Plugin as your are not using ARMember', 'ARMember'))));
                 die;
            }
            // $plugin = sanitize_text_field( $_POST['slug'] );//phpcs:ignore
            $plugin = $plugin_slug;
        ...
        function arm_plugin_install($plugin_name = '', $wpnonce_check = 1) {
            ...
            if( !empty($_POST['slug'] )){
                ...
                $plugin_slug_var = $plugin_name;
            }

            if (empty($plugin_slug_var)) {//phpcs:ignore
                wp_send_json_error(array(
                    'slug' => '',
                    'errorCode' => 'no_plugin_specified',
           ...
           if(!str_contains($plugin_slug , 'armember'))
           {
                echo json_encode(array('error' => 'false', 'msg' => addslashes(esc_html__('You cannot install this Plugin as your are not using ARMember', 'ARMember'))));
                die;
           }
            ...
            $status = array(
                'install' => 'plugin',
                'slug' => sanitize_key(wp_unslash($plugin_slug)),//phpcs:ignore
        ...
        function arm_plugin_api($res, $action, $args) {
            ...
            if (isset($_SESSION['arm_member_addon']) && !empty($_SESSION['arm_member_addon'])) {
                ...
                $armember_final_addons_array = array();
                $armember_final_addons_array = array_merge($armember_final_addons_array, $_SESSION['arm_member_addon']['feature']);
                $armember_final_addons_array = array_merge($armember_final_addons_array, $_SESSION['arm_member_addon']['payment_gateways']);
                $armember_final_addons_array = array_merge($armember_final_addons_array, $_SESSION['arm_member_addon']['integrations']);

                //$armember_addons = $_SESSION['arm_member_addon'];
                
                $obj = array();
                foreach ($armember_final_addons_array as $slug => $armember_addon) {
        ...
        function arm_get_social_settings() {
            ...
            if (!empty($social_settings['options'])) {
                ...
                $options['facebook']['label'] = esc_html__('Facebook', 'ARMember');
                $options['twitter']['label'] = esc_html__('Twitter', 'ARMember');
                $options['linkedin']['label'] = esc_html__('LinkedIn', 'ARMember');
                $options['vk']['label'] = esc_html__('VK', 'ARMember');
                $options['tumblr']['label'] = esc_html__('Tumblr', 'ARMember');
                $options['insta']['label'] = esc_html__('Instagram', 'ARMember');
                $options['google']['label'] = esc_html__('Google', 'ARMember');
        ...
        function arm_update_social_settings_func() {
            ...
            if (isset($post_data['s_action']) && $post_data['s_action'] == 'arm_update_social_settings') {
                ...
                $response = array('type' => 'success', 'msg' => esc_html__('Social Setting(s) has been Saved Successfully.', 'ARMember'));
            } else {
        ...
        function arm_update_social_network_from_form_func() {
            $response = array('type' => 'error', 'msg' => esc_html__('There is an error while updating settings, please try again.', 'ARMember'), 'old_settings' => '');
            global $wp, $wpdb, $ARMember, $arm_slugs, $arm_global_settings,$arm_capabilities_global;
            if (isset($_POST['action']) && $_POST['action'] == 'arm_update_social_network_from_form') {//phpcs:ignore
                ...
                $posted_data = array_map( array( $ARMember, 'arm_recursive_sanitize_data'), $_POST['arm_social_settings']['options'] );//phpcs:ignore
                $socialOptions = isset($posted_data) ? $posted_data : array();
                    ...
                    foreach ($socialOptions as $snk => $snv) {
                        if (!empty($snv)) {
                            ...
                            if (!empty($snv['custom_icon'])) {
                                ...
                                {
                                    $baseName = basename($snv['custom_icon']);
                                    if (isset($snv['icon']) && $snv['icon'] == 'custom') {
                                        $snv['icon'] = $baseName;
                                    }
                                    $icons[$baseName] = $snv['custom_icon'];
                                    update_option('arm_social_icons_' . $snk, $icons);
                                }
                            }
                        }
                ...
                $response = array('type' => 'success', 'msg' => esc_html__('Social Setting(s) has been Saved Successfully.', 'ARMember'), 'old_settings' => maybe_serialize($socialOptions));
            }
            echo arm_pattern_json_encode($response);
            die();
        }
        ...
        function arm_get_user_id_by_meta($meta_key = '', $meta_value = '') {
            ...
            if (!empty($meta_key) && !empty($meta_value)) {
                $user_id = $wpdb->get_var( $wpdb->prepare("SELECT `user_id` FROM `$wpdb->usermeta` WHERE `meta_key`=%s AND `meta_value`=%s",$meta_key,$meta_value) );
            }
            return $user_id;
        }
        ...
        function arm_social_login_shortcode_func($atts, $content, $tag) {
            ...
            global $wp, $wpdb, $ARMember, $arm_slugs, $arm_global_settings, $arm_social_feature;
                ...
                if (!empty($social_options)) {
                    foreach ($social_options as $sk => $so) {
                        ...
                        if (isset($icons[$so['icon']])) 
                        {
                            ...
                            */
                            $icon_img = '<img src="' . esc_url($icons[$so['icon']]) . '" alt="' . esc_attr($so['label']) . '" class="arm_social_login_custom_image">';
                        } else {
                            ...
                            */
                            $icon_img = '<img src="' . esc_url($icon_url) . '" alt="' . esc_attr($so['label']) . '" class="arm_social_login_custom_image">';
                        }
                        $content .= '<div class="arm_social_link_container arm_social_'.esc_attr($sk).'_container" id="arm_social_'.esc_attr($sk).'_container">';
                        $redirect_to = isset($redirect_to) ? $redirect_to : '';


                        $content .= '<input type="hidden" data-id="arm_social_login_redirect_to" value="' . esc_attr($redirect_to) . '">';
                        ...
                        switch ($sk) {
                            case 'facebook':
                                ...
                                $a_tag_attr = ' href="javascript:void(0)" onclick="FacebookLoginInit();" title="' . esc_attr__('Login With Facebook', 'ARMember') . '" ';
                                break;
                            case 'linkedin':
                                $authUrl = $arm_global_settings->add_query_arg('action', 'arm_login_with_linkedin', ARM_HOME_URL);
                                $arm_linkedin_client_id = $so['client_id'];
                                
                                $a_tag_attr = ' href="javascript:void(0)" onclick="LinkedInLoginInit();" title="' . esc_attr__('Login With LinkedIn', 'ARMember') . '" ';
                                
                                $content .= '<input type="hidden" data-id="arm_social_linkedin_client_id" value="' .
                                 esc_attr($arm_linkedin_client_id) . '">';
                                 $content .= '<input type="hidden" id="arm_social_linkedin_access_token" value="">';
                                $content .= '<input type="hidden" data-id="arm_social_linkedin_login_auth_url" value="' . esc_attr($authUrl) . '">';
                                break;
                            case 'twitter':
                                ...
                                $a_tag_attr = ' href="#" data-url="' . esc_attr($authUrl) . '" title="' . esc_attr__('Login With Twitter', 'ARMember') . '" ';
                                break;
                            case 'pinterest':
                                $content_js .= "jQuery(document).ready(function () {PinterestInit('" . $so['app_id'] . "');});";
                                $a_tag_attr = ' href="javascript:void(0)" onclick="PinterestLoginInit();" title="' . esc_attr__('Login With Pinterest+', 'ARMember') . '"';
                                break;
                            case 'vk':
                                ...
                                $content_js .= '';
                                $a_tag_attr = ' href="javascript:void(0)" id="arm_social_link_vk" onclick="VKAuthRequest();" title="' . esc_attr__('Login With vkontakte', 'ARMember') . '" ';
                                break;
                            case 'insta':
                                ...
                                $a_tag_attr = ' href="javascript:void(0)" onclick="InstagramLoginInit();" title="' . esc_attr__('Login With Instagram', 'ARMember') . '" ';
                                break;
                                case 'google':
                                    global $arm_version;
                                    ...
                                    $a_tag_attr = ' href="javascript:void(0)" onclick="GoogleSigninInit();" title="' . esc_attr__('Login With Google', 'ARMember') . '" ';
                                ...
                                $a_tag_attr = ' href="#" data-url="' . $authUrl . '" title="' . esc_attr__('Login With Tumblr', 'ARMember') . '" ';
                                break;
                            default:
                                break;
                        }
        ...
        function arm_social_login_callback($posted_data = array(), $verified_flag=0) {
            ...
            if($arm_social_feature->isSocialLoginFeature)
            {
                ...
                $or_posted_data['action'] = isset($_POST['action']) ? sanitize_text_field( $_POST['action'] ) : '';//phpcs:ignore
                $or_posted_data['action_type'] = isset($_POST['action_type']) ? sanitize_text_field( $_POST['action_type'] ) : '';//phpcs:ignore
                $or_posted_data['token'] = isset($_POST['token']) ? sanitize_text_field( $_POST['token'] ) : '';//phpcs:ignore
                $or_posted_data['fbuserId'] = isset($_POST['fbuserId']) ? sanitize_text_field( $_POST['fbuserId'] ) : '';//phpcs:ignore
                
                $posted_data = (!empty($posted_data)) ? $posted_data : $or_posted_data;
                $fail_msg = (!empty($arm_global_settings->common_message['social_login_failed_msg'])) ? $arm_global_settings->common_message['social_login_failed_msg'] : esc_html__('Login Failed, please try again.', 'ARMember');
                $fail_msg = (!empty($fail_msg)) ? $fail_msg : esc_html__('Sorry, Something went wrong. Please try again.', 'ARMember');
                ...
                if (!empty($posted_data) && $posted_data['action'] == 'arm_social_login_callback') {
                    ...
                    if (!empty($action_type)) {
                        ...
                        else if ($action_type == 'linkedin')
                        {
                            ...
                            $user_result=wp_remote_get('https://api.linkedin.com/v2/userinfo', $header);
                            
                            if(!empty($user_result['body']) && !is_wp_error($user_result))
                            {
                                $user_result_data = json_decode($user_result['body'], true);
                                
                                $ln_ac_id=$user_result_data['sub'];
                                if(!empty($ln_ac_id))
                                {
                                    $ln_first_name=(isset($user_result_data['given_name']))?$user_result_data['given_name']:'';
                                    $ln_last_name=(isset($user_result_data['family_name']))?$user_result_data['family_name']:'';
                                    $ln_profileurl=(isset($user_result_data['picture']))?$user_result_data['picture']:'';

                                    $posted_data['user_email'] = $user_result_data['email'];
                        ...
                        else if($action_type == 'insta')
                        {
                            ...
                            $data = $OBJ_INSTAGRAM->getOAuthToken($code);
                        ...
                        {
                            $error_msg = array( 'status' => 'error', 'message' => esc_html__('Something went wrong. Please try again.', 'ARMember') );
                        ...
                        if (!empty($user_id) && $user_id != 0) {
                            ...
                            if ($login_redirection_rules_options['type'] == 'page') {
                                ...
                                $redirect_to = apply_filters('arm_modify_redirection_page_external', $redirect_to,$user_id,$form_redirect_id);
                            } else {
                            ...
                            if ($one_click_signup == 1 && $user_data['user_email'] != '') {
                                ...
                                if (!empty($social_setting_options['assign_default_plan']) && $arm_subscription_plans->isFreePlanExist($social_setting_options['assign_default_plan'])) {
                    ...
                    if (MEMBERSHIP_DEBUG_LOG == true) {
                        ...
                        $arm_case_types['shortcode']['message'] = esc_html__('Couldn\'t login with social network', 'ARMember');
		...
		require_once (MEMBERSHIP_LIBRARY_DIR . '/twitter/OAuth.php');
                require_once (MEMBERSHIP_LIBRARY_DIR . '/twitter/twitteroauth.php');
                $_SESSION['arm_tw_oauth_token'] = isset($_SESSION['arm_tw_oauth_token']) ? $_SESSION['arm_tw_oauth_token'] : sanitize_text_field( $_REQUEST['oauth_token'] );//phpcs:ignore
		$oauth_verifier = isset($_GET['oauth_verifier']) ? sanitize_text_field($_GET['oauth_verifier']) : '';
                ...
                if (isset($user_info->error) || !isset($user_info->id) || empty($oauth_verifier)) {
                    if (MEMBERSHIP_DEBUG_LOG == true) {
                        ...
                        $arm_case_types['shortcode']['message'] = esc_html__('Couldn\'t login with twitter', 'ARMember');
                        $ARMember->arm_debug_response_log('arm_twitter_login_callback', $arm_case_types, $user_info, $wpdb->last_query, false);
                    }
                    echo "<script data-cfasync='false'>alert('" . esc_html__('There is an error while connecting twitter, Please try again.', 'ARMember') . "');window.close();</script>";
                    exit;
                } else {
                ...
                if ($slc_return['status'] == 'success') {
                    ...
                    window.opener.location.href='" . esc_attr($redirect_url) . "';window.close();
                    </script>";
                    exit;
                } else {
                    $fail_msg = (!empty($arm_global_settings->common_message['social_login_failed_msg'])) ? $arm_global_settings->common_message['social_login_failed_msg'] : esc_html__('Login Failed, please try again.', 'ARMember');
                    $fail_msg = (!empty($fail_msg)) ? $fail_msg : esc_html__('Sorry, Something went wrong. Please try again.', 'ARMember');
                    echo "<script data-cfasync='false'>alert('" . esc_html($fail_msg) . "');window.close();</script>";
                    exit;
                }
            }
            return;
        ...
        function arm_login_with_google_signin()
        {
            ...
            if (isset($_GET['arm_google_action']) && in_array($_GET['arm_google_action'], array('arm_google_signin_response')) && !empty($arm_social_feature->isSocialLoginFeature) ) {
                ...
                $access_token = isset($_GET['code']) ? sanitize_text_field($_GET['code']) : '';
                ...
                if( !is_wp_error($arm_google_auth_response) && isset($arm_google_auth_response['access_token'])) {
                    ...
                    echo "<script type='text/javascript'>
                            if( window.opener.document.getElementById('arm_social_google_access_token') != null ){
                                window.opener.document.getElementById('arm_social_google_access_token').value = '". esc_attr($access_token) ."';
                            } 
                            window.close();
                        </script>";   
                }
                ...
                {
                    echo "<script data-cfasync='false'>alert('" . esc_html__('There is an error while connecting Google SignIn, Please try again.', 'ARMember') . "');window.close();</script>";
                }
                die();
            }
        }
	...
	function arm_google_login_callback()
        ...
        function arm_login_with_linkedin() {
            ...
            if (isset($_GET['action']) && in_array($_GET['action'], array('arm_login_with_linkedin')) && !empty($arm_social_feature->isSocialLoginFeature) ) {
                ...
                $arm_redirect_url = $arm_global_settings->add_query_arg('action', sanitize_text_field($_GET['action']), ARM_HOME_URL);
                $api_params = array(
                    'grant_type' => 'authorization_code',
                    'code' => $access_token,
                    ...
                    $expire_time = $response_body['expires_in'];
                     ...
                     echo "<script type='text/javascript'>
                            if( window.opener.document.getElementById('arm_social_linkedin_access_token') != null ){
                                window.opener.document.getElementById('arm_social_linkedin_access_token').value = '". esc_attr($access_token) ."';
                            } 
                            window.close();
                        </script>";
                ...
                {
                    echo "<script data-cfasync='false'>alert('" . esc_html__('There is an error while connecting linkedin, Please try again.', 'ARMember') . "');window.close();</script>";
                }
                die();
            }
        }
        ...
        function arm_linkedin_login_callback() {
            ...
            if( !empty($arm_social_feature->isSocialLoginFeature) )
            {
                ...
                $arm_linkedin_get_url = 'https://api.linkedin.com/v2/userinfo';
        ...
        function arm_tumblr_login_callback() {
            ...
            if (isset($_REQUEST['page']) && in_array($_REQUEST['page'], array('arm_tumblr_return')) && !empty($arm_social_feature->isSocialLoginFeature) ) {
                ...
                if(isset($access_token['oauth_token']) && $access_token['oauth_token_secret']){             
                    ...
                    echo "<script data-cfasync='false'>alert('" . esc_html__('There is an error while connecting tumblr, Please try again.', 'ARMember') . "');window.close();</script>";
                }                
                if(isset($user_info->errors) && !isset($user_info->meta)) {
                    if (MEMBERSHIP_DEBUG_LOG == true) {
                        $arm_case_types['shortcode']['protected'] = true;
                        $arm_case_types['shortcode']['type'] = 'login_via_tumblr';
                        $arm_case_types['shortcode']['message'] = esc_html__('Couldn\'t login with tumblr', 'ARMember');
        ...
        function CheckpluginStatus($mypluginsarray, $pluginname, $attr, $purchase_addon, $plugin_type, $install_url, $compatible_version, $armember_version, $is_config = false, $config_url = '') {
            ...
            if( isset( $is_config ) && true == $is_config ){
                ...
                $config_button_html = '<a href="'.esc_url($config_url).'" class="arm_feature_configure_btn">Configure</a>';
            } else {
            ...
            if ($response == "NOT INSTALLED FREE") {
                $myicon = '<div class="arm_feature_button_activate_wrapper arm_install_btn "><a id="arm_free_addon" href="javascript:void(0);"  class="arm_feature_activate_btn" data-name=' . $purchase_addon . ' data-plugin=' . $arm_plugin_name[0] . '  data-href="javascript:void(0);" data-version="'.$compatible_version.'" data-arm_version="'.$armember_version.'" data-type ="free_addon">Install</a><span class="arm_addon_loader">
                    ...
                    $myicon .= '<div class="arm_feature_button_activate_wrapper hidden_section">
                        <a class="arm_feature_activate_btn arm_active_addon" data-file="' . esc_attr($pluginname) . '" href="javascript:void(0);"  data-version="'.$compatible_version.'" data-arm_version="'.esc_attr($armember_version).'" data-type ="activate_addon">Activate</a><span class="arm_addon_loader">
                                <svg class="arm_circular" viewBox="0 0 60 60">
                                    <circle class="path" cx="25px" cy="23px" r="18" fill="none" stroke-width="4" stroke-miterlimit="7"></circle>
                                </svg>
                            </span></div>';
                ...
                $myicon .= '<div class="arm_feature_button_activate_wrapper hidden_section">
                        <a class="arm_feature_activate_btn arm_active_addon" data-file="' . esc_attr($pluginname) . '" href="javascript:void(0);"  data-version="'.esc_attr($compatible_version).'" data-arm_version="'.esc_attr($armember_version).'" data-type ="activate_addon">Activate</a><span class="arm_addon_loader">
                                <svg class="arm_circular" viewBox="0 0 60 60">
                                    <circle class="path" cx="25px" cy="23px" r="18" fill="none" stroke-width="4" stroke-miterlimit="7"></circle>
                                </svg>
                            </span></div>';
                ...
                $myicon .= '<div class="arm_feature_button_deactivate_wrapper hidden_section"><a id="arm_feature_deactivate_btn" class="'.esc_attr($deactivate_btn_class).' arm_deactive_addon" data-file="' . esc_attr($pluginname) . '" href="javascript:void(0);"  data-version="'.esc_attr($compatible_version).'" data-arm_version="'.esc_attr($armember_version).'" data-type ="deactivate_addon">Deactivate</a>'.$config_button_html.'<span class="arm_addon_loader">
                                <svg class="arm_circular" viewBox="0 0 60 60">
                                    <circle class="path" cx="25px" cy="23px" r="18" fill="none" stroke-width="4" stroke-miterlimit="7"></circle>
                                </svg>
                            </span></div>';
        ...
        function addons_page( $force_check = 0 ) {
           ...
           if(false === $armember_addons_page || $force_check == 1) {
                ...
                if (is_wp_error($raw_response) || $raw_response['response']['code'] != 200) {
                    return "0|^^|<div class='error_message' style='margin-top:100px; padding:20px;'>" . esc_html__("Add-On listing is currently unavailable. Please try again later.", 'ARMember') . "</div>";
                } else {
        ...
        function arm_install_plugin_install_status($api, $loop = false) {
            ...
            if (isset($_GET['from']))
                $url .= '&amp;from=' . urlencode(wp_unslash(sanitize_text_field($_GET['from'])));
