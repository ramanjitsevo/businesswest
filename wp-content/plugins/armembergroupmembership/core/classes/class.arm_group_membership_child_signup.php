	...
	if (!class_exists('ARM_GROUPMEMBERSHIP_CHILD_SIGNUP')) 
	{
		class ARM_GROUPMEMBERSHIP_CHILD_SIGNUP
		{
			...
			{	
				...
				//Add Invite Code Field ID in signup form
				add_filter( 'arm_change_field_setting_external', array($this, 'arm_gm_change_field_setting_func'), 10, 2);
				...
				add_filter('arm_allow_specific_user_restricted_access', array($this, 'arm_gm_allow_child_user_restricted_access'), 10, 3);
				...
				add_action('arm_payment_gateway_validation_from_setup', array($this, 'arm_gm_plan_check_auto_debit_or_not'), 9, 4);
				...
				add_filter('arm_automated_message_email_content_filter', array($this, 'arm_automated_message_email_content_filter_for_group_membership'),10,6);

				add_filter('arm_general_settings_section_filters',array($this,'arm_general_settings_section_filters_func'),10,2);
			}


			function arm_general_settings_section_filters_func($arm_settings_menus=''){
				$arm_settings_menus .= '<a class="arm_general_settings_tab arm_padding_top_15 arm_padding_left_0 arm_padding_bottom_0 arm_padding_right_0" href="javascript:void(0)" data-target="#child_user_restrict_sec">'. esc_html__( 'Child User Restrict Page', 'ARMGroupMembership' ).'</a>';
				return $arm_settings_menus;
			}
			...
			function arm_gm_restrict_setup_form_invite_code($coupon_code)
			{
				...
				{
					$arm_gm_coupon_data = $wpdb->get_row($wpdb->prepare("SELECT * FROM `" . $ARMember->tbl_arm_coupons . "` WHERE `arm_coupon_code`=%s", $coupon_code)); //phpcs:ignore

					$arm_gm_parent_user_id = isset($arm_gm_coupon_data->arm_group_parent_user_id) ? $arm_gm_coupon_data->arm_group_parent_user_id : '';
			...
			function arm_gm_parent_and_child_user_filter_shortcode_content($content, $arm_gm_parent_user_id, $arm_gm_signup_posted_data)
			{
				...
				if(!empty($content) && !empty($arm_gm_parent_user_id) && !empty($arm_gm_signup_posted_data))
				{
					...
					if(!empty($arm_gm_userdata->arm_parent_user_id))
					{
						$arm_parent_user_data = $arm_gm_userdata->arm_parent_user_id;
						$arm_gm_parent_user_id = $arm_parent_user_data['arm_parent_user_id'];
						$arm_gm_userdata = get_userdata($arm_gm_parent_user_id);
						$arm_gm_parent_user_name = isset($arm_gm_userdata->user_login) ? $arm_gm_userdata->user_login : '';
					}
					...
					$arm_gm_signup_invite_code = isset($arm_gm_signup_posted_data['arm_gm_child_invite_code'])?$arm_gm_signup_posted_data['arm_gm_child_invite_code']:'';
					$content = str_replace('{ARM_GM_INVITE_COUPON_CODE}', $arm_gm_signup_invite_code, $content);

					$content = str_replace('{ARM_MESSAGE_USERNAME}', $arm_gm_parent_user_name, $content);
				}
			...
			function arm_gm_check_invited_code_user($armform)
			{
				...
				if(!empty($_POST) && !empty($_POST['arm_gm_child_invite_code'])) //phpcs:ignore
				{
					$arm_global_common_messages = isset($arm_global_settings->common_message) ? $arm_global_settings->common_message : array();

					$arm_gm_posted_invited_user_email = !empty($_POST['user_email']) ? $_POST['user_email'] : ''; //phpcs:ignore
					$arm_gm_child_invite_code = !empty($_POST['arm_gm_child_invite_code']) ? $_POST['arm_gm_child_invite_code'] : ''; //phpcs:ignore

					$arm_gm_check_coupon_exist = $wpdb->get_row("SELECT arm_coupon_id, arm_group_parent_user_id FROM ".$ARMember->tbl_arm_coupons." WHERE arm_coupon_code = '".$arm_gm_child_invite_code."'"); //phpcs:ignore

					$arm_group_parent_user_id = !empty($arm_gm_check_coupon_exist->arm_group_parent_user_id) ? $arm_gm_check_coupon_exist->arm_group_parent_user_id : 0;
					$arm_gm_check_user_invited_or_not = $wpdb->get_row("SELECT arm_gm_invite_code_id, arm_gm_status, COUNT(arm_gm_id) as total_cnt FROM ".$wpdb->prefix."arm_gm_child_users_status WHERE arm_gm_email_id = '".$arm_gm_posted_invited_user_email."' AND arm_gm_parent_user_id ='".$arm_group_parent_user_id."'"); //phpcs:ignore
			...
			{
				global $wpdb, $ARMember, $arm_global_settings, $arm_manage_communication, $arm_group_membership,$arm_version;
				...
				if(!empty($posted_data) && !empty($posted_data['arm_gm_child_invite_code']))
				{
					...
					$arm_gm_invite_code = $_POST['arm_gm_child_invite_code']; //phpcs:ignore
					$arm_gm_coupon_data = $wpdb->get_row($wpdb->prepare("SELECT * FROM `" . $ARMember->tbl_arm_coupons . "` WHERE `arm_coupon_code`=%s AND `arm_group_parent_user_id` IS NOT NULL", $arm_gm_invite_code)); //phpcs:ignore
					if(!empty($arm_gm_coupon_data) && !empty($arm_gm_coupon_data->arm_group_parent_user_id))
					{
						...
						$arm_parent_user_id = $arm_gm_coupon_data->arm_group_parent_user_id;

						$arm_gm_parent_user_plans = get_user_meta($arm_gm_coupon_data->arm_group_parent_user_id, 'arm_user_plan_ids', true);
            			$arm_gm_parent_user_plan = (isset($arm_gm_parent_user_plans) && !empty($arm_gm_parent_user_plans)) ? implode(',', $arm_gm_parent_user_plans) : 0;

						$arm_gm_is_child_user_data = array('plan_id' => $arm_gm_parent_user_plan);
						...
						$wpdb->query("UPDATE ".$wpdb->prefix."arm_gm_child_users_status SET arm_gm_user_id = '".$user_id."', arm_gm_status='1', arm_gm_updated_date = '".$arm_gm_updated_date."' WHERE arm_gm_email_id = '".$arm_gm_child_user_email."'"); //phpcs:ignore
						...
						if(!empty($arm_gm_parent_user_msg))
						{
							foreach($arm_gm_parent_user_msg as $arm_gm_parent_user_msg_key => $arm_gm_parent_user_msg_val)
							{
								if (!empty($arm_version) && version_compare($arm_version, '6.7.1', '>=') ) {
									$send_mail = $arm_manage_communication->arm_common_send_email($arm_gm_parent_user_msg_val,$arm_gm_parent_user_email,$arm_parent_user_id,$arm_gm_parent_user_plan, '',array(),$posted_data);
								}else{
									...
									$subject = $arm_manage_communication->arm_filter_communication_content($content_subject, $arm_parent_user_id, $arm_gm_parent_user_plan, '');
									$subject = $this->arm_gm_parent_and_child_user_filter_shortcode_content($subject, $arm_parent_user_id, $posted_data);

									$message = $arm_manage_communication->arm_filter_communication_content($content_description, $arm_parent_user_id, $arm_gm_parent_user_plan, '');

									$message = $this->arm_gm_parent_and_child_user_filter_shortcode_content($message, $arm_parent_user_id, $posted_data);

									$admin_message = $arm_manage_communication->arm_filter_communication_content($admin_content_description, $arm_parent_user_id, $arm_gm_parent_user_plan, '');
									$admin_message = $this->arm_gm_parent_and_child_user_filter_shortcode_content($admin_message, $arm_parent_user_id, $posted_data);
						...
						if(!empty($arm_gm_child_user_msg))
						{
							foreach($arm_gm_child_user_msg as $arm_gm_child_user_msg_key => $arm_gm_child_user_msg_val)
							{
								if (version_compare($arm_version, '6.7.1', '>=') ) {
									$send_mail = $arm_manage_communication->arm_common_send_email($arm_gm_child_user_msg_val,$arm_gm_child_user_email,$user_id,$arm_gm_parent_user_plan, '',array(),$posted_data);
								}else{
			...
			function arm_gm_change_field_setting_func($field, $field_options)
			{
				if( isset($field_options['id']) && $field_options['id']=='arm_gm_child_invite_code' ) 
				{
					$field['arm_form_field_id'] = $field_options['id'];
				}
				return $field;
			}
			...
			function arm_gm_change_field_option($form, $atts)
			{
				...
				if(!empty($atts['is_child']) && ($atts['is_child'] == 1))
				{
					...
					{*/
						...
						{
							$arm_txt_field_val = $_GET['arm_invite_code']; //phpcs:ignore
						}
			...
			{
				ob_start();
				require(ARM_GROUP_MEMBERSHIP_VIEWS_DIR.'/admin/arm_gm_shortcode_options.php');
				$content = ob_get_clean();
				echo $content; //phpcs:ignore
			}


			function arm_gm_allow_child_user_restricted_access( $current_user_plan, $user_id, $user_meta_key = 'arm_user_plan_ids' )
			{
				if(metadata_exists('user', $user_id, 'arm_parent_user_id'))
                {
                    ...
                    $parent_user_plans_ids = get_user_meta($arm_parent_user_id, $user_meta_key, true);
			...
			function arm_gm_child_user_page_restrict_view($general_settings)
			{
				...
				ob_start();
				require(ARM_GROUP_MEMBERSHIP_VIEWS_DIR.'/admin/arm_gm_child_user_page_restrict_field.php');
				$content = ob_get_clean();
				echo $content; //phpcs:ignore
			}
			...
			function arm_gm_plan_change_check_group_membership($post_data, $user_ID)
			{
				...
				$arm_gm_coupon_count = $wpdb->get_row($wpdb->prepare("SELECT COUNT(arm_coupon_id) as total_rec FROM `" . $ARMember->tbl_arm_coupons . "` WHERE `arm_group_parent_user_id`=%d AND arm_coupon_used != 0", $user_ID)); //phpcs:ignore
                ...
                if($arm_gm_total_coupons > 0)
                {
					...
					else
					{
						...
						if((!empty($post_data['arm_user_plan']) || !empty($post_data['subscription_plan'])) && (!$is_multiple_membership_feature->isMultipleMembershipFeature))
						{
		                    ...
		                    if(empty($arm_gm_plan_obj->isPaidPost) && !$arm_gm_plan_obj->exists() )
		                    {
				                $arm_gm_coupon_count = $wpdb->get_row($wpdb->prepare("SELECT COUNT(arm_coupon_id) as total_rec FROM `" . $ARMember->tbl_arm_coupons . "` WHERE `arm_group_parent_user_id`=%d AND arm_coupon_used != 0", $user_ID)); //phpcs:ignore
				                $arm_gm_total_coupons = $arm_gm_coupon_count->total_rec;
			    ...
			    else if(!empty($post_data['arm_user_plan']) && !empty($post_data['arm_gm_selected_user']))
                {
                	...
                	*/
            		...
            		$arm_gm_plan_max_members = isset($arm_gm_plan_options['arm_gm_max_members']) ? $arm_gm_plan_options['arm_gm_max_members'] : 0;
		    		$arm_gm_plan_min_members = isset($arm_gm_plan_options['arm_gm_min_members']) ? $arm_gm_plan_options['arm_gm_min_members'] : 0;
		    		$arm_gm_plan_sub_user_seat_slot = isset($arm_gm_plan_options['arm_gm_sub_user_seat_slot']) ? $arm_gm_plan_options['arm_gm_sub_user_seat_slot'] : 0;

					$arm_gm_selected_sub_user = isset($post_data['arm_gm_selected_user'][0]) ? $post_data['arm_gm_selected_user'][0] : 0;
					...
					echo "<pre>arm_gm_plan_id=>";
					print_r($arm_gm_plan_id);

					echo "<pre>arm_gm_plan_max_members=>";
					print_r($arm_gm_plan_max_members); */
			...
			function arm_gm_users_list_count_func($atts, $content, $tag)
			{
	            ...
	            $atts = shortcode_atts(array(
         		...
         		{
         			$arm_gm_count_total_invite_codes = $wpdb->get_row("SELECT COUNT(arm_coupon_id) as total_rec FROM `" . $ARMember->tbl_arm_coupons . "` WHERE `arm_group_parent_user_id` = ".$arm_gm_current_user_id.""); //phpcs:ignore

         			$arm_gm_total_invite_codes = $arm_gm_count_total_invite_codes->total_rec;

         			$arm_gm_count_purchased_invite_codes = $wpdb->get_row("SELECT COUNT(arm_coupon_id) as total_rec FROM `" . $ARMember->tbl_arm_coupons . "` WHERE `arm_group_parent_user_id` = ".$arm_gm_current_user_id." AND arm_coupon_used = 1"); //phpcs:ignore
			...
			function arm_automated_message_email_content_filter_for_group_membership($content,$user_id,$plan_id,$follwer_id,$mail_addtional_data){

				global $arm_manage_communication,$arm_group_membership;
				$content = $arm_manage_communication->arm_filter_communication_content($content, $user_id, $plan_id, '');
				if(isset($mail_addtional_data['arm_gm_coupon_id']) && isset($mail_addtional_data['arm_gm_coupon_code'])){
					$content = $arm_group_membership->arm_gm_filter_shortcode_content($content, $mail_addtional_data['arm_gm_coupon_code'], $mail_addtional_data['arm_gm_coupon_id']);
				}else{
					$content = $this->arm_gm_parent_and_child_user_filter_shortcode_content($content, $user_id, $mail_addtional_data);
				}
