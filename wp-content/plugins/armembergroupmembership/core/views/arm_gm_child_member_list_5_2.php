<div class="<?php echo esc_attr($arm_gm_group_membership_container_class); ?>_loader_img"></div>
<div class="<?php echo esc_attr($arm_gm_group_membership_container_class); ?>">
<?php
    $frontfontstyle = $arm_global_settings->arm_get_front_font_style();
    $google_font_url = isset($frontfontstyle['google_font_url']) ? $frontfontstyle['google_font_url'] : '';
?>
<link id="group_membership_google_font" rel="stylesheet" type="text/css" href="<?php echo esc_url($google_font_url); ?>" />
...
<style type="text/css">
	...
	$arm_gm_css_string .= ".arm_gm_child_user_parent_wrapper_container .arm_invite_user_button, .arm_gm_child_user_parent_wrapper_container .arm_gm_resend_email_button, .arm_gm_child_user_parent_wrapper_container .arm_delete_user_button { {$frontfontstyle['frontOptions']['button_font']['font']} }";
        ...
        echo $arm_gm_css_string; //phpcs:ignore
    ?>
</style>
...
<?php
    ...
    $default_form_id = !empty($arm_gm_shortcode_form_id) ? $arm_gm_shortcode_form_id : $arm_member_forms->arm_get_default_form_id('registration');
    ...
    if ($form->exists() && !empty($form->fields)) 
    {
        ...
        $form_style_class .= ($form_style['rtl'] == '1') ? ' arm_form_rtl' : ' arm_form_ltr arm-default-form';
    ...
    // Invite User Modal Code Starts
        $arm_gm_body_content .= '<div id="arm_gm_form_invite_user_shortcode_modal" class="arm_gm_invite_modal arm_msg_pos_bottom " style="width:550px;">';
        ...
        $arm_gm_body_content .= '<span class="popup_header_text">' . $popup_title . '</span>'; //phpcs:ignore
        ...
        $arm_gm_body_content .= '<div class="arm_setup_messages arm_form_message_container arm_editor_form_fields_container arm_editor_form_fields_wrapper" style="display:none">';
        $arm_gm_body_content .= "<div class='arm-df__fc--validation__wrap'><ul><li>".$arm_gm_user_invitation_success_msg." <i class='armfa armfa-times'></i></li></ul></div>";
        $arm_gm_body_content .= '</div>';

        $arm_gm_body_content .= '<div class="armclear"></div>';

        $arm_gm_body_content .='<form method="post" name="arm_form_gm_invite" id="arm_invite_form_' . $setupRandomID . '" class="arm_invite_form_id arm_setup_form_' . $form_id . ' arm_invite_setup_form  ' . $form_style_class . '" enctype="multipart/form-data" data-random-id="' . $setupRandomID . '"  novalidate>';
        ...
        $arm_gm_pass_hidden_fields = array(
            ...
            'arm_gm_membership_field_username' => $arm_gm_membership_field_username,
            'arm_gm_membership_field_name' => $arm_gm_membership_field_name,
            'arm_gm_membership_field_email' => $arm_gm_membership_field_email,
            'arm_gm_membership_field_status' => $arm_gm_membership_field_status,
            'arm_gm_membership_field_action' => $arm_gm_membership_field_action,
        );
        ...
        $arm_gm_body_content .='<div class="arm-df-wrapper arm_msg_pos_bottom">';
        ...
        foreach ($arm_invite_fields_array as $key) 
        {
            ...
            switch ($key) {
                case 'email_label':
                    ...
                    if(!empty($arm_allow_notched_outline))
                    {
                        ...
                        $ffield_label_html .= '<label class="arm-df__label-text" for="arm-df__form-control_'.$key.'"> * ' . html_entity_decode(stripslashes($popup_field_label)) . '</label>';
                    ...
                    else
                    {
                        ...
                        $ffield_label = '<label class="'.$class_label.'" for="arm-df__form-control_'.$key.'_'.$setupRandomID.'"> * ' . html_entity_decode(stripslashes($popup_field_label)) . '</label>';
                    }
                    
                    $fieldAttr = 'name="' . $type . '[' . $key . ']"';
                    $fieldAttr .='required=""';
                    $fieldAttr .= ' data-validation-required-message="' . esc_html__('This field can not be left blank', 'ARMGroupMembership') . '" data-validation-regex-regex="^.+@.+\..+$" data-validation-regex-message="'. esc_html__('Please enter valid email address.','ARMGroupMembership').'"';  
                    $validation .= '<div class="arm-df__fc--validation"></div>';
            ...
            $fieldtable .="<div class='arm-df__form-field'>";
            $fieldtable .="<div class='arm-df__form-field-wrap_text arm-df__form-field-wrap arm-controls'>";
            $fieldtable .= $ffield_label;
            $fieldtable .="<input type='text' class='arm-df__form-control arm-df__form-control_".$key." arm_material_input  ".$fieldClass."' 
                             value=''  id='arm-df__form-control_".$key."_".$setupRandomID."' ".$fieldAttr.">";
            $fieldtable .="</div>".$validation."";
        }



        $arm_gm_body_content .= '<div class="arm_cc_fields_container arm_' . $type . '_fields arm-df__fields-wrapper arm_front_side_form arm_field_position_' . $fieldPosition . '" >';
        $arm_gm_body_content .= '<span class="payment-errors"></span>';
        $arm_gm_body_content .= '<div class ="arm-control-group arm-df__form-group arm-df__form-group_text">';
        $arm_gm_body_content .= $fieldtable;
        $arm_gm_body_content .= '</div>';
        $arm_gm_body_content .= '</div>';
        $arm_gm_body_content .= '<div class="armclear"></div>';
        $arm_gm_body_content .= '<div class ="arm-control-group arm-df__form-group arm-df__form-group_submit">';
        $arm_gm_body_content .='<div class="arm_label_input_separator"></div>';
        $arm_gm_body_content .='<div class="arm-df__form-field">';
        $arm_gm_body_content .='<div class="arm-df__form-field-wrap_submit arm-df__form-field-wrap arm-controls" id="arm_invite_form' . $form_id . '">';
        ...
        $arm_gm_body_content .='<button type="submit" name="arm_gm_invite_btn" class="arm-df__form-control-submit-btn arm-df__form-group_button  arm-waves-effect arm-waves-light arm_material_input ' . $btn_style_class . '"  ' . $ngClick . ' id="arm_invite_invite_submit" data-current_page="'.$current_page.'" data-per_page="'.$per_page.'"><span class="arm_spinner">' . $arm_loader_img . '</span>'.$popup_button_text.'</button>';
    ...
    $arm_gm_body_content .= '<div class="arm_setup_messages arm_form_message_container arm_editor_form_fileds_container arm_editor_form_fileds_wrapper"></div>';
    ...
    //Invite Button Code
        ...
        $arm_gm_body_content .= "<div><button type='button' class='arm_gm_child_member_list_invite_btn arm_invite_user_button' ".$arm_gm_hide_button.">".$invite_users_button_txt."</button></div>";
    ...
    if(!empty($child_users_list))
    {
        ...
        foreach($child_users_list as $arm_gm_child_users_key => $arm_gm_child_users_val)
        {
            ...
            $arm_gm_fetch_coupon_data = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$ARMember->tbl_arm_coupons." WHERE arm_coupon_id = %d", $arm_gm_child_users_val->arm_gm_invite_code_id)); //phpcs:ignore


            $arm_gm_body_content .= "<tr class='arm_group_membership_list_item' id='arm_group_membership_tr_" . $sr_no . "'>";

            $arm_gm_body_content .= "<td data-label='".$arm_gm_membership_field_username."' class='arm_group_membership_list_item_plan_sr'>".$arm_gm_username."</td>";

            $arm_gm_body_content .= "<td data-label='".$arm_gm_membership_field_name."' class='arm_group_membership_list_item_plan_sr'>".$arm_gm_first_name.' '.$arm_gm_last_name."</td>";

            $arm_gm_body_content .= "<td data-label='".$arm_gm_membership_field_email."' class='arm_group_membership_list_item_plan_sr'>".$arm_gm_user_email."</td>";



            $arm_gm_body_content .= "<td data-label='".$arm_gm_membership_field_status."' class='arm_group_membership_list_item_plan_sr'>";
            $arm_gm_body_content .= "<div>".$arm_gm_status_text."<br>".__('Code:', 'ARMGroupMembership')." ";
            if(empty($arm_gm_child_users_val->arm_gm_status))
            {
                $arm_gm_body_content .= "<div class='arm_shortcode_text arm_form_shortcode_box'><span class='armCopyText'>".$arm_gm_fetch_coupon_data->arm_coupon_code."</span><span class='arm_click_to_copy_text' data-code='".$arm_gm_fetch_coupon_data->arm_coupon_code."'>Click to copy</span><span class='arm_copied_text'><img src='".MEMBERSHIPLITE_IMAGES_URL."/copied_ok.png' alt='ok'>Code Copied</span>".$arm_gm_fetch_coupon_data->arm_coupon_code."";
            }
            else
            {
            ...
            $arm_gm_body_content .= "<td data-label='".$arm_gm_membership_field_action."' class='arm_group_membership_list_item_plan_sr' style='text-align: left;'>";
            $arm_gm_body_content .= "<div class='arm_group_membership_action'>";
    ...
    echo $arm_gm_body_content; //phpcs:ignore
