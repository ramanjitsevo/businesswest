jQuery(document).ready(function(){
    wp.hooks.addAction('arm_membership_plans_fields','armembergroupmembership',arm_gm_membership_plans_fields_func,10);
    wp.hooks.addAction('arm_membership_plans_fields_close_action','armembergroupmembership',arm_membership_plans_fields_close_action_func,10);   
});
function arm_membership_plans_fields_close_action_func() {
    jQuery('.arm_gm_sub_opt').addClass('hidden_section');
}
function arm_gm_membership_plans_fields_func(response)
{
    jQuery('#arm_gm_group_membership_disable_referral').prop('checked',false).trigger('change');
    plan_options = response.arm_plan_options;   
    var parent_wrapper = jQuery('#arm_gm_group_membership_disable_referral').parents('.page_sub_content');
    parent_wrapper.find('.arm_addon_plan_settings').removeClass('arm_margin_bottom_28');
    parent_wrapper.find('.arm_addon_plan_settings').addClass('arm_margin_bottom_5');
    if(plan_options.arm_gm_enable_referral == 1 || plan_options.arm_gm_enable_referral == '1' )
    {
        jQuery('#arm_gm_group_membership_disable_referral').prop('checked',true).trigger('change');
        parent_wrapper.find('.arm_addon_plan_settings').removeClass('arm_margin_bottom_5');
        parent_wrapper.find('.arm_addon_plan_settings').addClass('arm_margin_bottom_28');
    }
    jQuery('#gm_min_members').val(plan_options.arm_gm_min_members);
    jQuery('#gm_max_members').val(plan_options.arm_gm_max_members);
    jQuery('#gm_sub_user_seat_slot').val(plan_options.arm_gm_sub_user_seat_slot);
    if(response.arm_plan_type == "recurring")
    {
        jQuery(".arm_group_membership_recurring_error").removeClass('hidden_section');
    }
    else
    {
        jQuery(".arm_group_membership_recurring_error").addClass('hidden_section');   
    }
}
...
function arm_disable_save_plan_details_btn(){
    jQuery('.arm_submit_btn_container .arm_save_btn').attr('disabled', 'disabled');
}

function arm_enable_save_plan_details_btn(){
    jQuery('.arm_submit_btn_container .arm_save_btn').removeAttr('disabled', 'disabled');
}

jQuery(document).on('keypress', '#arm_group_membership_div #gm_max_members, #arm_group_membership_div #gm_min_members, #arm_group_membership_div #gm_sub_user_seat_slot', function(evt){
...
jQuery(document).on('blur', '#arm_group_membership_div #gm_max_members, #arm_group_membership_div #gm_min_members', function(e){
    ...
    if((arm_gm_min_member_val != "") && (arm_gm_max_member_val != "") && (arm_gm_max_member_val < arm_gm_min_member_val))
    {
        ...
        arm_disable_save_plan_details_btn();
    }
    else if(arm_gm_max_member_val == 0 || arm_gm_min_member_val == 0){
        if(arm_gm_min_member_val == 0){
            jQuery(".arm_zero_min_member_error").fadeIn().css('display','inline-block');
        }else{
            jQuery(".arm_zero_min_member_error").fadeOut();
        }
        
        if(arm_gm_max_member_val == 0){
            jQuery(".arm_zero_max_member_error").fadeIn().css('display','inline-block');
        }else{
            jQuery(".arm_zero_max_member_error").fadeOut();
        }
    ...
    else
    {
        ...
        arm_enable_save_plan_details_btn();
    }
});
...
jQuery(document).on('change', "#arm_gm_group_membership_disable_referral", function () {
    ...
    if(isChecked){
        parent_wrapper.find('.arm_addon_plan_settings').removeClass('arm_margin_bottom_5');
        parent_wrapper.find('.arm_addon_plan_settings').addClass('arm_margin_bottom_28');
        parent_wrapper.find('.arm_gm_sub_opt').removeClass('hidden_section');
    } else {
        parent_wrapper.find('.arm_addon_plan_settings').addClass('arm_margin_bottom_5');
        parent_wrapper.find('.arm_addon_plan_settings').removeClass('arm_margin_bottom_28');
        parent_wrapper.find('.arm_gm_sub_opt').addClass('hidden_section');
        jQuery('.arm_user_plan_save_btn').removeAttr('disabled');
    }
    gm_min_members = jQuery('#gm_min_members').val();
    gm_max_members = jQuery('#gm_max_members').val();
    gm_sub_user_seat_slot = jQuery('#gm_sub_user_seat_slot').val();
    if(gm_min_members == '')
    {
        jQuery('#gm_min_members').val(1);
    }
    if(gm_max_members == '')
    {
        jQuery('#gm_max_members').val(1);
    }
    if(gm_sub_user_seat_slot == '')
    {
        jQuery('#gm_sub_user_seat_slot').val(1);
    }
});
...
jQuery(document).on('click', '.arm_gm_child_users', function () {
    ...
    if (arm_gm_parent_user_id != '' && arm_gm_parent_user_id != 0) {
        ...
        arm_popup_initialize('#arm_gm_child_users_data',160,0,'slideDown');
...
jQuery(document).on('click','.arm_gm_child_user_cls_btn',function(){
    jQuery('#arm_gm_child_users_data').bPopup({
...
function showDeleteConfirmation(item_id) {
    if (item_id != '') {
        ...
        deleteBox.addClass('armopen').fadeToggle();
...
jQuery(document).on('click', '.arm_gm_openpreview_popup', function () { 
    ...
    if (member_id != '' && member_id != 0) {
        ...
        jQuery.ajax({
            type: "POST",
            url: __ARMAJAXURL,
            data: "action=arm_member_view_detail&member_id=" + member_id + "&view_type=popup&_wpnonce="+nonce,
            success: function (response) {
                if (response != '') {
...
jQuery(document).on('click', '.arm_member_edit_gm_cancel_btn', function(e){
    jQuery('#arm_gm_edit_users_data').bPopup().close();
});
...
jQuery(document).on('click', '.arm_member_edit_gm_save_btn', function(e){
    ...
    jQuery('.arm_loader_img_edit_child_seat_gm').fadeIn('slow');
    jQuery.ajax({
        type: "POST",
        url: __ARMAJAXURL,
        ...
        success: function (response) {
            setTimeout(function () {
                ...
                jQuery('.arm_loader_img_edit_child_seat_gm').fadeOut();
                jQuery('#armember_datatable').dataTable().fnDestroy();
                arm_load_gm_list_grid(false);                
            }, 800);
        }
    ...
    if (iKeyCode != 46 && iKeyCode > 31 && (iKeyCode < 48 || iKeyCode > 57) && iKeyCode != 37 && iKeyCode != 39) {
        ...
        if (value != '') {
            ...
            if (result.length >= 1 && iKeyCode == 46) {
                ...
                {
                    var addded_member = (parseInt(value) + parseInt(AlreadyPurchased))
                    if(addded_member > MaximumLimit)
                    {
...
{
    jQuery('#arm_gm_add_sub_user_form')[0].reset();
    jQuery('#arm_gm_add_sub_users_data .arm_loading').fadeIn('slow');
    var bPopup = jQuery('#arm_gm_add_sub_users_data').bPopup({
        opacity: 0.5,
        follow: [false, false],
    });
    jQuery('#arm_gm_add_sub_user_form .arm_gm_display_input_error').each(function()
    {
        jQuery('.arm_gm_username_required_error').hide();
        jQuery('.arm_gm_username_error').hide();
        jQuery('.arm_gm_email_required_error').hide();
        jQuery('.arm_gm_email_error').hide();
        jQuery('.arm_gm_pass_required_error').hide();
...
jQuery(document).on('click', '.arm_member_add_sub_user_gm_cancel_btn', function(e){
    jQuery("#arm_gm_add_sub_users_data").bPopup().close();
    jQuery('#arm_gm_add_sub_users_data .arm_gm_display_input_error').each(function()
    {
        jQuery('.arm_gm_username_required_error').hide();
        jQuery('.arm_gm_username_error').hide();
        jQuery('.arm_gm_email_required_error').hide();
...
jQuery(document).on('click', '.arm_member_add_sub_user_gm_save_btn', function(e){
    ...
    jQuery('.arm_loader_img_add_sub_user_gm').show();
    ...
    jQuery.ajax({
        ...
        success: function (response) {
            ...
            if(response_data.status == 0)
            {
                ...
                jQuery('.arm_loader_img_add_sub_user_gm').hide();
            }
            else
            {
                jQuery('.arm_loader_img_add_sub_user_gm').hide();
...
function arm_gm_delete_child_users(delete_user_id, user_status = 'active')
{
    ...
    jQuery.ajax({
        ...
        success: function(result){
            ...
            setTimeout(function () {
                jQuery('#arm_gm_child_users_data').bPopup().close();
                jQuery('#armember_datatable').dataTable().fnDestroy();
...
jQuery(document).on("change", "#arm_import_user_form #arm_plan_id", function() {
    var selected_plan = jQuery(this).val();
    jQuery(".arm_loader_img_import_user").show();
    jQuery.ajax({
        type: "POST",
        url: __ARMAJAXURL,
        data: "action=arm_gm_display_child_user_seat_selection&plan_id=" + selected_plan,
        success: function (response) {
            jQuery(".arm_loader_img_import_user").hide();
            jQuery("#arm_import_user_form .arm_gm_child_user_selection_form").remove();
            var arm_res = JSON.parse(response);
            if(arm_res.status == 1)
            {
                jQuery('#arm_import_user_form #arm_plan_id').parent().append(arm_res.content);
                arm_selectbox_init();
            }
        }
    });
