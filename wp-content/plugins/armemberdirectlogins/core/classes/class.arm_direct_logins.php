...
if(!class_exists('ARM_Member_Direct_Logins')){
    ...
    class ARM_Member_Direct_Logins{
        ...
        function arm_direct_logins_grid_data() {
            ...
            $sorting_ord = isset($_REQUEST['sSortDir_0']) ? $_REQUEST['sSortDir_0'] : 'desc'; //phpcs:ignore
            $sorting_col = (isset($_REQUEST['iSortCol_0']) && $_REQUEST['iSortCol_0'] > 0) ? intval($_REQUEST['iSortCol_0']) : 5;
            $order_by = 'user_login';
            if( $sorting_col == 1 ) {
                $order_by = 'user_email';
            ...
            $number = isset($_REQUEST['iDisplayLength']) ? intval($_REQUEST['iDisplayLength']) : 10;
        ...
        function arm_direct_logins_get_remaining_time( $expire_time ) {
            ...
            foreach ($a as $secs => $str) {
                ...
                if ($d >= 1) {
                    $r = round($d);
                    return esc_html__(sprintf('%d %s remaining', $r, ($r > 1 ? $a_plural[$str] : $str)), 'ARM_DIRECT_LOGINS'); //phpcs:ignore
                }
            }
        }
        ...
        function arm_direct_logins_update_status() {
            ...
            //if (current_user_can('administrator')) {
                update_user_meta( $_REQUEST['arm_dl_user_id'], 'arm_direct_logins_enable', $_REQUEST['arm_dl_status']); //phpcs:ignore
                if($_REQUEST['arm_dl_status']) //phpcs:ignore
                    $status_inword = __('Active', 'ARM_DIRECT_LOGINS');
                else
                    $status_inword = __('Inactive', 'ARM_DIRECT_LOGINS');
                
                $response = array( 'type'=>'success', 'user_id'=> $_REQUEST['arm_dl_user_id'], 'dl_status'=>$_REQUEST['arm_dl_status'], 'dl_status_inword' => $status_inword ); //phpcs:ignore
            //}
            echo json_encode($response);
            die;
        }
        ...
        function arm_direct_login_remove() {
            ...
            //if (current_user_can('administrator')) {
                delete_user_meta( $_REQUEST['user_id'], 'arm_direct_logins_enable'); //phpcs:ignore
                delete_user_meta( $_REQUEST['user_id'], 'arm_direct_logins_expire_time'); //phpcs:ignore
                delete_user_meta( $_REQUEST['user_id'], 'arm_direct_logins_token'); //phpcs:ignore
                delete_user_meta( $_REQUEST['user_id'], 'arm_direct_logins_user', true); //phpcs:ignore
        ...
        function arm_direct_logins_get_all_members($type = 0, $only_total = 0) {
            ...
            $users_details = $wpdb->get_results($user_query); //phpcs:ignore
        ...
        function arm_direct_logins_tokens() 
        {
            ...
            if (!empty($_GET['armdl_token'])) 
            {
                if(!is_user_logged_in())
                {
                    ...
                    $armdl_token = $_GET['armdl_token']; //phpcs:ignore
                    ...
                    if (empty($user)) {
                        ...
                        update_user_meta( $user_id, 'arm_direct_logins_flag', 1);

                        //$redirect_to = admin_url();
                        $redirect_to = !empty( $_REQUEST['redirect_to'] ) ? $_REQUEST['redirect_to'] : admin_url(); //phpcs:ignore
                ...
                else 
                {
                    ...
                    if (!empty($user_id)) {
                        $this->arm_dl_check_is_expired_login($user_id);
                    }
                }
            }
            ...
            {
                $user_id = get_current_user_id();
                $arm_direct_logins_flag = get_user_meta( $user_id, 'arm_direct_logins_flag', true);
                if (!empty($user_id) && !empty($arm_direct_logins_flag)) 
                {
                    $this->arm_dl_check_is_expired_login($user_id);
                }
        ...
        function arm_dl_check_is_expired_login($user_id)
        {
            ...
            if(!empty($check) && $check == '1')
            {
                ...
                if(!empty($expire) && current_time('timestamp') <= floatval($expire) && !empty($enable) && $enable == '1')
                {
                    ...
                    if ((!empty($page) && in_array($page, $bloked_pages)) || (!empty($pagenow) && in_array($pagenow, $bloked_pages))) {
                        wp_die(esc_html__("You don't have permission to access this page", 'ARM_DIRECT_LOGINS' ));
                    }
                }
                else
                {
                    update_user_meta( $user_id, 'arm_direct_logins_flag', 0);
        ...
        function arm_direct_login_add_history_for_set_logged_in_cookie($auth_cookie, $expire, $expiration, $user_id, $scheme) {
            ...
            $select_result = $wpdb->get_var($select_query); //phpcs:ignore
            ...
            $update_query = $wpdb->prepare("UPDATE `{$tbl_login_history}` SET `arm_user_current_status` = %d  WHERE `arm_user_current_status` != %d AND `arm_user_id` = %d AND `arm_history_browser` = %s AND `arm_logged_in_ip` = %s", 0, 0, $user_id, $browser_detail, $logged_in_ip); //phpcs:ignore
            $update_result = $wpdb->query($update_query); //phpcs:ignore
            $insert_query = $wpdb->prepare("INSERT INTO `{$tbl_login_history}` (`arm_user_id`,`arm_logged_in_ip`,`arm_logged_in_date`,`arm_history_browser`,`arm_history_session`,`arm_login_country`,`arm_user_current_status`) VALUES (%d,%s,%s,%s,%s,%s,%d)", $user_id, $logged_in_ip, $logged_in_time, $browser_detail, $browser_session_id, $country,1); //phpcs:ignore
            $insert_result = $wpdb->query($insert_query); //phpcs:ignore
            ...
            if(is_user_logged_in() && is_admin())
            {
                ...
                if ($autolock_shared_account == 1) {
                    ...
                    if (isset($_COOKIE['arm_dl_autolock_cookie_'.$user_id]) && !empty($_COOKIE['arm_dl_autolock_cookie_'.$user_id])) {
                        ...
                        $arm_autolock_cookie = $_COOKIE['arm_dl_autolock_cookie_'.$user_id];  //phpcs:ignore
                        ...
                        $update_query = $wpdb->prepare("UPDATE `{$login_history_table}` SET `arm_logout_date` = %s, `arm_user_current_status` = %d WHERE `arm_history_id` != %d AND `arm_history_session` != %s AND `arm_user_id` = %d AND `arm_user_current_status` != %d", $logged_out_time, 0, $arm_history_id, $arm_session_id, $user_id, 0);  //phpcs:ignore
        ...
        function arm_direct_logins_footer() {
            global $ARMember;
            echo $ARMember->arm_get_need_help_html_content('arm_direct_login_doc'); //phpcs:ignore
        }
        ...
        function get_direct_logins_member_list_func(){
            if(isset($_REQUEST['action']) && $_REQUEST['action']=='get_direct_logins_member_list') {
                ...
                $text = $_REQUEST['txt']; //phpcs:ignore
                $type = 0;
