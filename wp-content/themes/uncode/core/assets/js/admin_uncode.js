	...
	$.fn.uncode_init_upload = function() {
		...
		$(document).on('click', '.ot_upload_media', function(e) {
			...
			var uncode_frames = {}, // Store our workflows in an object
				...
				media = wp.media,
				field_id = $(this).closest('.option-tree-ui-upload-parent, .field-media-uploader').find('input').attr('id'),
						...
						$.ajax({
							...
							success: function(data) {
								if (!isNaN(data.id) && data != undefined) {
									...
									$('#' + field_id).parent().parent('div').append('<span class="option-tree-ui-media-wrap" id="' + field_id + '_media" />');
			...
			uncode_frames[frame_id].on('select', function() {
				var attachment = uncode_frames[frame_id].state().get('selection').first(),
					...
					regex = /^image\/(?:jpe?g|png|url|gif|svg\+xml|x-icon)$/i;
				if (mime == 'oembed/svg') {
				...
				$('#' + field_id + '_media').remove();
				if ( $('#' + field_id).closest('label').length ) {
					$('#' + field_id).closest('label').addClass('has-media').append('<span class="option-tree-ui-media-wrap" id="' + field_id + '_media" />');
				} else {
					$('#' + field_id).parent().parent('div').addClass('has-media').append('<span class="option-tree-ui-media-wrap" id="' + field_id + '_media" />');
				}
				$('#' + field_id + '_media').append(btnContent).slideDown();
		...
		$(document).on('click', '.option-tree-ui-remove-media', function(e) {
			...
			var cont = $(e.currentTarget).closest('td'),
				$cont_2 = $(e.currentTarget).closest('.field-media-uploader'),
				$label = $(e.currentTarget).closest('label.has-media')
				$input = cont.find('.option-tree-ui-upload-parent input'),
				$input_2 = $cont_2.find('input'),
				$placeholder = cont.add($cont_2).find('.option-tree-ui-media-wrap');
			$input.add($input_2).attr('value','').val('');
			$label.removeClass('has-media');
			$placeholder.remove();
		});
	};
