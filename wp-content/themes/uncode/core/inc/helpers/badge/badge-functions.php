...
function uncode_print_menu_badge_item( $item ) {
	...
	if ( ! empty( $item->badge_text ) ) {
		...
		if ( substr( $badge_color, 0, 1 ) === "#" ) {
			$badge_color_exa = sanitize_hex_color( $badge_color );
			$style_text          .= 'color:' . $badge_color_exa . ';';
		} else {
			if ( isset( $front_background_colors[$badge_color] ) ) {
				$badge_color_exa = $front_background_colors[$badge_color];
				$classes[]       = 'text-' . $badge_color . '-color';
			}
		}
