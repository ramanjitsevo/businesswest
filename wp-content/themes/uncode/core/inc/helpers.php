...
function uncode_get_menu_mobile_block() {
	global $post;
	$menu_block_id = ot_get_option('_uncode_menu_mobile_block');
	if ($post && isset($post->ID)) {
		if ( get_post_meta( $post->ID, '_uncode_specific_menu_mobile_block', true ) !== '' ) {
			if ( get_post_meta( $post->ID, '_uncode_specific_menu_mobile_block', true ) === 'none' ) {
				$menu_block_id = false;
			} else {
				$menu_block_id = get_post_meta( $post->ID, '_uncode_specific_menu_mobile_block', true );
			}
		}
	}
		 
	return apply_filters( 'uncode_get_menu_mobile_block', $menu_block_id );
}