	...
	if (!empty($fonts) && is_array($fonts)) {
		foreach ($fonts as $key => $value) {
			...
			if ($font_name !== '') {
				...
				echo "\n" . '----------------------------------------------------------*/';
				echo "\n" . '.' . $font_class . ',';
				echo "\n" . '.btn-custom-typo.' . $font_class . ',';
				echo "\n" . '.menu-container .' . $font_class . ' a { font-family: ' . $font_name . ' !important; }';
			}
...
if (isset($uncode_option['_uncode_heading_font_heights'])) {
	...
	if (!empty($font_heights) && is_array($font_heights)) {
		foreach ($font_heights as $key => $value) {
			...
			echo "\n" . '----------------------------------------------------------*/';
			echo "\n" . '.' . $value['_uncode_heading_font_height_unique_id'] . ' { line-height: ' . $value['_uncode_heading_font_height'] . '!important; }';
			echo "\n" . '#masthead .' . $value['_uncode_heading_font_height_unique_id'] . ' { line-height: ' . $value['_uncode_heading_font_height'] . '!important; }';
		}
	}
}
...
if ($default_font_size !== '') {
	if ( ( $default_font_size > 0 && is_numeric( $default_font_size ) ) || str_replace(floatval($default_font_size), '', $default_font_size) === 'px' ) {
		echo "\n" . 'body,p,li,dt,dd,dl,address,label,pre,code,.nav-tabs .tab-excerpt, .block-wrapper-parent { font-size: ' . $default_font_size . 'px; }';
	} else {
		echo "\n" . 'body,p,li,dt,dd,dl,address,label,pre,code,.nav-tabs .tab-excerpt, .block-wrapper-parent { font-size: ' . $default_font_size . '; }';
	}
}
...
$color_heading_alpha_05 = 'rgba('.$_color_heading_alpha[0].','.$_color_heading_alpha[1].','.$_color_heading_alpha[2].',0.5)';

$color_heading_inverted = $cs_heading_color_dark;
$_color_heading_inverted_alpha = function_exists('uncode_hex2rgb') ? uncode_hex2rgb($color_heading_inverted) : array(0,0,0);
$color_heading_inverted_alpha_05 = 'rgba('.$_color_heading_inverted_alpha[0].','.$_color_heading_inverted_alpha[1].','.$_color_heading_inverted_alpha[2].',0.5)';
