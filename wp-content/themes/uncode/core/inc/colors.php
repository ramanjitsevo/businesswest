...
function uncode_print_style_custom_colors_css( $key, $value, $value_rgb, $btn_outline ) {
	if ( strpos( $value, 'background' ) !== false || strpos( $value, 'after' ) !== false ) {
 ...
 */
function uncode_print_style_custom_solid_color_css( $key, $value, $value_rgb, $btn_outline, $properties = array( 'bg', 'button', 'text', 'border', 'overlay', 'icon', 'after', 'hover', 'active' ) ) {
	// Background
	...
	// Icon
	if ( is_array( $properties ) && in_array( 'icon', $properties ) ) {
		echo "\n" . '.icon-' . $key . ' { color: ' . $value . ' !important; }';
	}

	// After
	if ( is_array( $properties ) && in_array( 'after', $properties ) ) {
		echo "\n" . '.after-bg-' . $key . '-color:after { background-color: ' . $value . ' !important;}';
	}

	// Text
	if ( is_array( $properties ) && in_array( 'text', $properties ) ) {
		echo "\n" . '.text-' . $key . '-color, .text-' . $key . '-color a { color: ' . $value . ' !important; fill: ' . $value . ' !important; }';
	}

	// Hover
	if ( is_array( $properties ) && in_array( 'hover', $properties ) ) {
		echo "\n" . '.hover-' . $key . '-color a:hover { color: ' . $value . ' !important; fill: ' . $value . ' !important; }';
	}

	// Active
	if ( is_array( $properties ) && in_array( 'active', $properties ) ) {
		echo "\n" . '.active-' . $key . '-color.active { color: ' . $value . ' !important; fill: ' . $value . ' !important; }';
		echo "\n" . '.submenu-light .menu-container .menu-smart .block-wrapper-parent .current-menu-ancestor > .unmenu-title.active-' . $key . '-color,';
		echo "\n" . '.submenu-dark .menu-container .menu-smart .block-wrapper-parent .current-menu-ancestor > .unmenu-title.active-' . $key . '-color,';
		echo "\n" . '.submenu-light .menu-container .menu-smart .block-wrapper-parent .current-menu-ancestor > .unmenu-title.active-' . $key . '-color a,';
		echo "\n" . '.submenu-dark .menu-container .menu-smart .block-wrapper-parent .current-menu-ancestor > .unmenu-title.active-' . $key . '-color a,';
		echo "\n" . '.submenu-light .menu-container .menu-smart .block-wrapper-parent .active-' . $key . '-color.current-menu-item,';
		echo "\n" . '.submenu-dark .menu-container .menu-smart .block-wrapper-parent .active-' . $key . '-color.current-menu-item,';
		echo "\n" . '.submenu-light .menu-container .menu-smart .block-wrapper-parent .active-' . $key . '-color.current-menu-item a,';
		echo "\n" . '.submenu-dark .menu-container .menu-smart .block-wrapper-parent .active-' . $key . '-color.current-menu-item a  { color: ' . $value . ' !important; fill: ' . $value . ' !important; }';
	}
 ...
 */
function uncode_print_style_custom_gradient_color_css( $key, $value, $properties = array( 'bg', 'button', 'text', 'border', 'after' ) ) {
	// Background
	if ( is_array( $properties ) && in_array( 'bg', $properties ) ) {
		echo "\n" . '.style-' . $key . '-bg { ' . $value . ' }';
		echo "\n" . '.after-bg-' . $key . '-color { ' . $value . ' }';
	}
