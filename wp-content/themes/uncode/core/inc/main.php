...
function uncode_equeue() {
	...
	if ( apply_filters( 'uncode_remove_query_strings', true ) && function_exists('get_rocket_option') && ( get_rocket_option( 'remove_query_strings' ) || get_rocket_option( 'minify_css' ) || get_rocket_option( 'minify_js' ) ) || ( function_exists('vc_is_page_editable') && vc_is_page_editable() ) ) {
	...
	$site_parameters = array(
		...
		'menuShowOnClick'			 => apply_filters( 'uncode_menu_show_on_click', ot_get_option( '_uncode_menu_show_on_click' ) === 'on' ),
	...
	if ((isset($main_width[0]) && $main_width[0] !== '') || (!is_array($main_width) && $main_width !== '')) {
		...
		.limit-width { max-width: " . $output_width . $output_unit . "; " . $main_align_css . "}
		.menu-container:not(.grid-filters) .menu-horizontal ul.menu-smart .megamenu-block-wrapper > .vc_row[data-parent=\"true\"].limit-width:not(.row-parent-limit) { max-width: calc(" . $output_width . $output_unit . " - 72px); }
		}";
	}
