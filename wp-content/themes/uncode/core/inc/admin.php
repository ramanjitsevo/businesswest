...
if ( ! $menu_item || ! is_object( $menu_item ) ) {
		return $menu_item;
	}
	...
	$menu_item->media = get_post_meta($menu_item->ID, '_menu_item_media', true);
	$menu_item->icon_color = get_post_meta($menu_item->ID, '_menu_item_icon_color', true);
	$menu_item->icon_bg = get_post_meta($menu_item->ID, '_menu_item_icon_bg', true);
...
function uncode_update_custom_nav_fields($menu_id, $menu_item_db_id, $args) {
	...
	if (isset($_REQUEST['menu-item-media']) && is_array($_REQUEST['menu-item-media'])) {
		$media_value = (array_key_exists($menu_item_db_id, $_REQUEST['menu-item-media'])) ? $_REQUEST['menu-item-media'][$menu_item_db_id] : '';
		update_post_meta($menu_item_db_id, '_menu_item_media', $media_value);
	} else {
		update_post_meta($menu_item_db_id, '_menu_item_media', '');
	}
	if (isset($_REQUEST['menu-item-icon-color']) && is_array($_REQUEST['menu-item-icon-color'])) {
		$icon_color_value = (array_key_exists($menu_item_db_id, $_REQUEST['menu-item-icon-color'])) ? $_REQUEST['menu-item-icon-color'][$menu_item_db_id] : '';
		update_post_meta($menu_item_db_id, '_menu_item_icon_color', $icon_color_value);
	} else {
		update_post_meta($menu_item_db_id, '_menu_item_icon_color', '');
	}
	if (isset($_REQUEST['menu-item-icon-bg']) && is_array($_REQUEST['menu-item-icon-bg'])) {
		$icon_bg_value = (array_key_exists($menu_item_db_id, $_REQUEST['menu-item-icon-bg'])) ? $_REQUEST['menu-item-icon-bg'][$menu_item_db_id] : '';
		update_post_meta($menu_item_db_id, '_menu_item_icon_bg', $icon_bg_value);
	} else {
		update_post_meta($menu_item_db_id, '_menu_item_icon_bg', '');
	}
...
function uncode_media_uploader_demand() {
	global $pagenow;
...
add_action( 'admin_footer', 'uncode_media_uploader_demand' );
...
if (!function_exists('uncode_get_post_types')) {
	function uncode_get_post_types($built_in = false) {
		...
		if (($key = array_search('popup_theme', $get_post_types)) !== false) {
	    	unset($get_post_types[$key]);
		}
		if (($key = array_search('wpb_gutenberg_param', $get_post_types)) !== false) {
	    	unset($get_post_types[$key]);
		}
		if ($built_in) {
		...
		if ( apply_filters( 'uncode_include_attachments_in_search', false ) && is_search() ) {
			$uncode_post_types[] = 'attachment';
		}

		return apply_filters( 'uncode_get_post_types', $uncode_post_types );
	}
}
