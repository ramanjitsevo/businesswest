...
if (!class_exists('unmenu')) {
	class unmenu {
		...
		function __construct($type, $param)
		{
			...
			$uncode_menu_mobile_block = $this->uncode_menu_mobile_block();
			$menu_w_mobile_block = $uncode_menu_mobile_block !== '' ? ' tablet-hidden mobile-hidden no-mobile' : '';
				...
				if ($menu_generic !== '') {
					...
					if (isset($theme_locations['primary'])) {
						...
						if (isset($menu_obj->name)) {
							...
							$primary_menu = apply_filters( 'uncode_get_default_primary_menu', $primary_menu );
						}
					}
				}
			}
			...
			if ($socials_active === 'on' || $search_active === 'on' || $woo_icon !== '' || $woo_icon_mobile !== '' || $login_account_icon !== '' || $login_account_icon_mobile !== '' || $woo_wishlist_icon !== '' || $woo_wishlist_icon_mobile !== '' || $social_html_inner !== '') {
				...
				if ( strpos( $menutype, 'hmenu' ) !== false ) {
					if ( $search_active === 'on' && $search_mobile === 'on' ) {
						$show_search_dropdown = false;
					}
					if ( ot_get_option( '_uncode_menu_li_animation' ) !== '' && ot_get_option( '_uncode_menu_li_animation' ) !== 'on' && ot_get_option( '_uncode_menu_li_animation' ) !== 'off' ) {	
						$data_masthead = ' data-menu-anim="' . ot_get_option( '_uncode_menu_li_animation' ) . '"';
					}
				}
			...
			switch ($type) {
				...
				case 'hmenu-justify':
					$this->html = '<div class="menu-wrapper'.$menu_shrink.$menu_sticky.$menu_no_arrow.'">
													...
													<header id="masthead" class="navbar'.$stylemaincombo.$main_absolute.$menu_sub_animation.' menu-with-logo' . $menu_mobile_off_cavas . '"' . $data_masthead . '>
																...
																<div class="row-menu-inner'.$stylemainback.'">
																	...
																	<div class="col-lg-12 main-menu-container middle">
																		<div class="menu-horizontal' . $sub_extra_classes . '">
																			<div class="menu-horizontal-inner' . $menu_w_mobile_block . '">
																				'.wp_nav_menu( array(
																					"menu"              => $primary_menu,
						...
						$this->html .=										'</div>
						' . $this->uncode_menu_mobile_block('menu-horizontal-inner') . '
																		</div>
																	</div>
																</div>
															</div>'
				...
				case 'hmenu-center':
					$this->html = '<div class="menu-wrapper'.$menu_sticky.$menu_no_arrow.$menu_sub_animation.' style-'.$stylemain.'-original">'.
													...
													<header id="masthead" class="navbar'.str_replace(' menu-transparent', '', $stylemaincombo).'"' . $data_masthead . '>
														<div class="menu-container'.$effects.$stylemainbackfull.$needs_after.'" role="navigation">
															<div class="row-menu'.$main_width.'">
																<div class="row-menu-inner'.$stylemainback.'">
																	<div class="col-lg-12 main-menu-container middle">
				...
				case 'hmenu-center-split':
					...
					$logo_html = '<div class="logo-container megamenu-diff middle">
												...
												$this->html = '<div class="menu-wrapper'.$menu_shrink.$menu_sticky.$menu_no_arrow.$menu_sub_animation.'">
													'.($no_secondary !== 'on' ? $secondary_menu_html : '').'
													<header id="masthead" class="navbar'.$stylemaincombo.$main_absolute.' menu-with-logo' . $menu_mobile_off_cavas . '"' . $data_masthead . '>
														<div class="menu-container'.$effects.$stylemainbackfull.$needs_after.'" role="navigation">
															<div class="row-menu'.$main_width.'">
																<div class="row-menu-inner'.$stylemainback.'">
				...
				case 'offcanvas_head':
					...
					$this->html = '<div class="menu-wrapper'.$menu_shrink.$menu_sticky.'">
													...
													<div id="masthead" class="navbar'.$stylemaincombo_overlay.$main_absolute.' menu-with-logo' . $menu_mobile_off_cavas . '"' . $data_masthead . '>
					...
					if ( $primary_menu_out != '' || $search !== '' || ( $no_secondary !== 'on' && $secondary_menu_out != '' ) ) {
														...
														<div class="main-header">
															<div class="vmenu-container menu-container style-'.$styleoverlay.$menu_no_arrow.$stylemaincombo.'" role="navigation" data-lenis-prevent>
																<div class="row row-parent">
																	<div class="row-inner">
																		<div class="menu-sidebar main-menu-container' . $menu_w_mobile_block . '">
						...
						if ($no_secondary !== 'on' && $secondary_enhanced !== 'on') {
							...
							$this->html .= 						'</div>
																		...
																		' . $this->uncode_menu_mobile_block('','main-menu-container') . '
																	</div>
																</div>
															</div>
														</div>
					...
					$this->html = '<div class="menu-wrapper'.$menu_shrink.$menu_sticky.$menu_no_arrow.'">
													...
													<header id="masthead" class="navbar'.$stylemaincombo.$main_absolute.$menu_sub_animation.' menu-with-logo' . $menu_mobile_off_cavas . '"' . $data_masthead . '>
														<div class="menu-container'.$effects.$stylemainbackfull.$needs_after.'" role="navigation">
															<div class="row-menu'.$main_width.'">
																<div class="row-menu-inner'.$stylemainback.'">
																	<div class="col-lg-5 main-menu-container middle' . $menu_w_mobile_block . '">
																		<div class="menu-horizontal' . $sub_extra_classes . '">
																			<div class="menu-horizontal-inner">
																				'.wp_nav_menu( array(
																					"menu"              => $primary_menu,
				...
				default:
					...
					$this->html = '<div class="main-header">
													<div id="masthead" class="masthead-vertical'.$menu_sticky.'"' . $data_masthead . '>
					...
					$this->html .= 				'<div class="row-inner expand">
																	<div class="main-menu-container' . $menu_w_mobile_block . '">
		...
		public function uncode_menu_mobile_block($class = '', $second = false) {
			$cblock = uncode_get_menu_mobile_block();
			$class .= $class !== '' ? ' ' : '';
			$return = '';
			if ( $cblock ) {
				$return .= '<div id="uncode-mobile-menu-block" aria-hidden="true" class="' . $class . 'desktop-hidden main-container menu-block-' . esc_attr($cblock) . '">';
				if ( $second !== false ) {
					$return .= '<div class="' . $second . '">';
				}
				$return .= get_post_field('post_content', $cblock);
				if ( $second !== false ) {
					$return .= '</div>';//$second
				}
				$return .= apply_filters( 'uncode_menu_mobile_block_before_close', false );
				$return .= '</div>';//#uncode-mobile-menu-block
				$return = preg_replace(
					'/(\[uncode_menu_block)(\s+)/',
					'$1 mobile="yes"$2',
					$return
