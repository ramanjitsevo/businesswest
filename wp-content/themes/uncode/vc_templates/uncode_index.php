...
$attributes_first = array(
	...
	'table_hover' => '',
	'inner_padding' => '',
	'post_items' => 'media|featured|onpost|original,title,date',
	'page_items' => 'media|featured|onpost|original,title',
	'product_items' => 'media|featured|onpost|original|hide-sale|inherit-atc|inherit-w-atc|atc-typo-default|show-atc,title,price|default',
	...
	'css_grid_equal_height_bottom' => '',
	'css_grid_equal_height_no_mobile' => '',
	'active_filters' => '',
	'clear_all' => '',
...
if (isset($uncode_post_types) && !empty($uncode_post_types)) {
	foreach ($uncode_post_types as $key => $value) {
		...
		$attributes_second[$value . '_items'] = $value === 'product' ? 'media|featured|onpost|original|hide-sale|inherit-atc|inherit-w-atc|atc-typo-default|show-atc,title,price|default' : 'media|featured|onpost|original,title';
...
global $uncode_query_options, $uncode_in_navbar;
$save_uncode_in_navbar = $uncode_in_navbar;
...
if ( $uncode_in_navbar && $filtering === 'ajax' ) {
	$filtering = '';
}
...
if ($index_type == 'isotope') {
	...
	if ( $advanced_nav === 'yes' ) {
		$carousel_nav = $carousel_nav_mobile = $carousel_dots = $carousel_dots_space = $carousel_dots_mobile = $carousel_dots_inside = $carousel_dot_position = $carousel_dot_width = $column_width_use_pixel = $carousel_width_pixel = '';
	}
	...
	if ( $css_grid_equal_height === 'yes' && $single_text === 'under' ) {
		...
		if ( $css_grid_equal_height_bottom === 'yes' ) {
			$container_classes[] = 'cssgrid-equal-height-bottom';
		}
		if ( $css_grid_equal_height_no_mobile === 'yes' ) {
			$container_classes[] = 'cssgrid-equal-height-no-mobile';
		}
	}
	...
	} elseif ( $titles_gap_reduced_mobile === 'no' ) {
		$main_container_classes[] = 'drop-mobile-no-gap';
	} 
...
} else if ( $index_type === 'css_grid' || $index_type === 'linear' ) {
...
if ( $pagination_scroll_disabled === 'yes' ) {
	$main_data['data-pagination-scroll'] = 'disabled';
}
...
<div<?php if ($index_type === 'isotope' || $index_type === 'css_grid') { echo ' id="' . esc_attr($el_id) .'"'; } ?> class="<?php echo esc_attr(trim(implode(' ', $main_container_classes))); ?>" <?php echo implode(' ', $main_data_attributes); ?>>
	<?php if ( ( $posts_counter > 0 || uncode_is_filtering() ) && ( $index_type === 'css_grid' || $index_type === 'isotope' ) ):  ?>
					...
					<div class="isotope-filters grid-filters menu-container <?php echo esc_attr( implode( ' ', $filters_container_class ) ); ?> menu-<?php echo esc_attr($filter_style); ?>">
						...
						<div class="menu-horizontal<?php echo esc_attr( ($filtering_full_width !== 'yes') ? ' limit-width' : '' ); ?><?php echo esc_attr( $menu_sub_enhanced ); ?> text-<?php echo esc_attr($filtering_position); ?> text-mobile-<?php echo esc_attr($filter_mobile_align); ?> filter-alignment-<?php echo esc_attr($filtering_position); ?> <?php echo esc_attr( ( $woo_sorting_shadow ) ? ' menu-dd-shadow-sm' : '' ); ?>">
							<?php if ( ! $filter_hide_cats ) : ?>
								...
								<?php } ?>
								<ul class="menu-smart sm<?php echo esc_attr( ($filtering_uppercase === 'yes') ? ' text-uppercase' : ' no-text-uppercase' ); ?> menu-smart--filter-cats <?php echo esc_attr( $filter_mobile_dropdown === 'yes' ? 'menu-smart--filter-cats-mobile-dropdown ul-mobile-dropdown' : '' ); ?>" role="menu">
									<?php
										global $wp;
									...
									<li role="menuitem" class="<?php echo esc_attr($show_all_class); ?>">
										<span>
											...
											?>
												<a href="#" role="button" data-filter="*" class="<?php if (!isset($_GET['ucat'])) echo 'active'; if ($filtering_uppercase !== 'yes') echo ' no-letterspace'; ?> isotope-nav-link grid-nav-link">
											<?php else: ?>
												<?php
												...
												?>
												<a role="button" href="<?php echo esc_url( $filter_url ); ?>" class="<?php if (!isset($_GET['ucat'])) echo 'active'; if ($filtering_uppercase !== 'yes') echo ' no-letterspace'; ?>  <?php echo esc_attr( $isotope_nav_link_class ); ?>">
											<?php
											endif;
									...
									<?php foreach ( $categories_array as $cat ):
										if ($cat->taxonomy !== 'product_type'): ?>
											<?php if (($infinite === 'yes' || $pagination !== 'yes' || $uncode_index_query->max_num_pages == 1) && !isset($_GET['ucat'])) : ?>
												<?php if ( apply_filters( 'uncode_standard_filters_show_term', true, $cat->term_id ) ) : ?><li role="menuitem" class="filter-cat-<?php echo esc_attr($cat->term_id); ?> filter-cat"><span><a role="button" href="#" data-filter="grid-cat-<?php echo esc_attr($cat->term_id); ?>" class="<?php if (isset($_GET['ucat']) && $_GET['ucat'] == $cat->term_id) echo 'active'; ?> isotope-nav-link grid-nav-link"><?php echo esc_attr( $cat->name ) ?></a></span></li><?php endif; ?>
											<?php else : ?>
												...
												<li role="menuitem" class="filter-cat-<?php echo esc_attr($cat->term_id); ?> filter-cat"><span><a role="button" href="<?php echo esc_url( $filter_url ); ?>" class="<?php if (isset($_GET['ucat']) && $_GET['ucat'] == $cat->term_id) { echo 'active'; } ?>  <?php echo esc_attr( $isotope_nav_link_class ); ?>"><?php echo esc_attr( $cat->name ) ?></a></span></li>
											<?php endif; ?>
										<?php endif;
									endforeach; ?>
								</ul>
						...
						$filtering_menu_out .= '<div class="filter-main-group' . $align_options . '">';
							...
							if ( $show_woo_sorting_ajax ) {
								...
								if ( $show_woo_result_count_ajax !== '' ) {
									$woo_result_wrapper_class = $show_woo_result_count_ajax === 'short' ? 'woocommerce-result-count-wrapper woocommerce-result-count-wrapper--short' : 'woocommerce-result-count-wrapper woocommerce-result-count-wrapper--default';
									$filtering_menu_out .= '<div class="' . $woo_result_wrapper_class . '">';
		...
		?>
		<?php if ($index_type === 'linear' && $draggable === 'yes' ) { ?>
			<div data-dragger>
		<?php } ?>
		<?php if ( !empty( $parent_container_classes ) ) { ?>
			<?php $div_data_cont_attributes = array_map(function ($v, $k) { return $k . '="' . $v . '"'; }, $div_data_cont, array_keys($div_data_cont)); ?>
			<div class="<?php echo esc_attr(trim(implode(' ', $parent_container_classes))); ?>" <?php echo implode(' ', $div_data_cont_attributes); ?><?php echo uncode_switch_stock_string( $max_w_ajax_grid_style ); ?>><?php } ?>
		...
		<?php $linear_count = 0;
			...
			if ( $posts_counter > 0 ):
				...
				foreach ( $posts as $key_post => $post ):
					...
					if ( apply_filters( 'uncode_include_attachments_in_search', false ) && $post->type === 'attachment' ) {
						$block_data['is_attachment'] = true;
					}
					...
					$use_legacy_check = apply_filters( 'uncode_use_legacy_tmb_media_check', false );

					if ($use_legacy_check) {
						// Old code logic
						if (isset($typeLayout['media']) && !array_key_exists('media',$typeLayout) && $single_text === 'overlay' && $style_preset === 'masonry') {
					...
					$lightbox_classes = apply_filters( 'uncode_index_lightbox_classes', $lightbox_classes, $block_data );
				...
				if ($pagination === 'yes'):
					...
					if (isset($_GET['ucat']) || is_front_page() || is_home() || is_archive() || is_single()) {
						...
						if ($paged > 1) {
							...
							if ( apply_filters( 'uncode_index_pagination_add_trailing_slash', false ) ) {
								$prev_link = trailingslashit( $prev_link );
							}
						}
						if ($paged == 2) {
						...
						if ($paged < $uncode_index_query->max_num_pages) {
							...
							if ( apply_filters( 'uncode_index_pagination_add_trailing_slash', false ) ) {
								$next_link = trailingslashit( $next_link );
							}
						}
					}
					...
					if (is_array($paginate_links)) :
						...
						<div class="<?php echo esc_attr( $index_type === 'css_grid' ? 'cssgrid' : $index_type ); ?>-footer-inner grid-footer-inner<?php if ($footer_full_width !== 'yes') { echo ' limit-width'; ?> menu-<?php echo esc_attr($footer_style); } ?> text-center">
							<ul class='pagination'>
								...
								<?php
								if ( apply_filters( 'uncode_index_pagination_add_trailing_slash', false ) && is_array( $paginate_links ) ) {
									foreach ( $paginate_links as $key => $link ) {
										// Only modify links that contain page numbers (not the first page)
										if ( strpos( $link, $format ) !== false || preg_match( '/\/page\/\d+["\']/', $link ) ) {
								            $paginate_links[$key] = preg_replace( '/(href="[^"]+)(")/i', '$1/$2', $link );
										}
									}
								}
								?>
