...
if ( $custom !== '' ) {
	$class .= 'custom-height';
	$custom_height .= is_numeric($custom_height) ? 'px' : '';
	$style = ' style="--custom-height:' . esc_html($custom_height) . '"';
} else {
	switch ($empty_h) {
		case '1':
			$class .= 'empty-half';
...
?>
<div class="<?php echo esc_attr(trim($css_class)); ?>" <?php echo uncode_switch_stock_string( $el_id ) . $style; ?>><span class="empty-space-inner"></span></div>
